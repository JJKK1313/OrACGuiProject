...
class ChooseACTypeScreen(QtWidgets.QDialog):
    ...
    def __init__(self, id_list):
        super(ChooseACTypeScreen, self).__init__()
        self.ui = ui_ACTypeDialogBox.Ui_Dialog()
        self.ui.setupUi(self)
        self.exist_id_list = id_list
        self.new_ac = AC()
        self.status = False  # is done successfully and data is valid
        self.ui.ElectraPushBtn.clicked.connect(lambda: self.save_ac_of_type(ELECTRA))
        self.ui.SamsungPushBtn.clicked.connect(lambda: self.save_ac_of_type(SAMSUNG))
        self.ui.CustomizePushBtn.clicked.connect(self.on_customize_click)
    ...
    def save_ac_of_type(self, ac_type_commands_dict):
        self.new_ac.commands = ac_type_commands_dict
        self.close()
